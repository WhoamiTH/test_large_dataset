...
def get_MLP_predict_result(test_data, ref_label, net):
    test_input_data_1 = torch.Tensor(torch.from_numpy(test_data).float())
    test_input_data_2 = torch.Tensor(torch.from_numpy(ref_label).float())
...
# 获取初步结果
test_result = get_MLP_predict_result(cur_test_data, ref_label, net)
